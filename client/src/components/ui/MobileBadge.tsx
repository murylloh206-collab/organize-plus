// client/src/components/ui/MobileBadge.tsx
import React from "react";

interface MobileBadgeProps {
  variant?: "success" | "warning" | "danger" | "info" | "neutral" | "primary" | "gold";
  size?: "sm" | "md";
  icon?: string;
  showDot?: boolean;
  className?: string;
...
const variantConfig: Record<string, { className: string; dot: string }> = {
...
export default function MobileBadge({ 
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full font-semibold ${config.className} ${sizeClasses[size]} ${className}`}>
      {showDot && (
        <span className={`size-1.5 rounded-full ${config.dot} flex-shrink-0`} />
      )}
      {icon && (
