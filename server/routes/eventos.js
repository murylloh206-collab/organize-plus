...
import { getEventosBySala, getEventosProximos, // Nova função
createEvento, updateEvento, deleteEvento } from "../storage.js";
const router = Router();
...
router.put("/:id", requireAdmin, async (req, res) => {
    try {
        ...
        const eventoData = {};
        if (titulo !== undefined)
            eventoData.titulo = titulo;
        if (descricao !== undefined)
            eventoData.descricao = descricao;
        if (data !== undefined)
            eventoData.data = new Date(data);
        if (local !== undefined)
            eventoData.local = local;
        if (tipo !== undefined)
            eventoData.tipo = tipo;
        if (status !== undefined) {
