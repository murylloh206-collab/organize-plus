...
import { getPagamentosBySala, getPagamentosByUsuario, getPagamentosPendentes, createPagamento, updatePagamento, confirmarPagamentoViaComprovante } from "../storage.js";
...
router.post("/confirmar-comprovante", requireAuth, (req, res) => {
    upload.single("comprovante")(req, res, async (err) => {
        try {
            ...
            const resultado = await confirmarPagamentoViaComprovante(parseInt(pagamentoId), comprovanteUrl, descricaoPagamento);
            res.json({
                success: true,
