...
import { getRifasBySala, createRifa, updateRifa, getTicketsByRifa, getTicketsByVendedor, createTicket, updateTicket, } from "../storage.js";
const router = Router();
// GET /api/rifas?salaId=
router.get("/", requireAuth, async (req, res) => {
    const salaId = parseInt(req.query.salaId || String(req.session.salaId));
    if (!salaId)
        return res.status(400).json({ message: "salaId obrigatório" });
